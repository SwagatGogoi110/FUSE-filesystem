...
static int fsmkdir(const char *path, mode_t mode){
	...
	strcpy(new_folder -> path, rindex + 1);
...
int fsaccess(const char *path, int mask){
	return 0;
}

int fsrename(const char* from, const char* to){
	printf("RENAME: %s\n", from);
	printf("RENAME: %s\n", to);
...
int fstruncate(const char *path, off_t size){
	return 0;
}
